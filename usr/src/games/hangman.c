...
int main(void)
{
    ...
    h_draw_board();
    h_mask(word, used);
    game_num(12, 8, (unsigned int)misses);
    while (misses < 7) {
        if (h_done(word, used)) {
            print_at(19, 0, "You solved it. Press q.");
            while (game_key() != 'q')
                h_turns++;
            return 0;
        }
        key = game_key_echo(15, 7);
        h_turns++;
        if (key == 'q')
            return 0;
        if (key < 'a' || key > 'z') {
    ...
    h_draw_stage(7);
    print_at(19, 0, "Hanged. The word was:");
    print_at(20, 0, word);
    print_at(22, 0, "Press q to leave.");
    while (game_key() != 'q')
        h_turns++;
    return 0;
}
